 ...
 *8.私信会出现接收到1条消息，但是显示0条新消息。
 ...
 1.个人背包中礼物数量。
 2.个人主页没头像没有默认头像。
 3.个人主页宠物皇冠角标，自己创建的排序在前。
 4.宠物主页口粮和礼物页面。
 
 2014.12.31
 1.banner点击网页网址添加&SID=。
 2.发布照片页键盘弹出后输入框位置变化。
 3.兑换页面默认宠物，口粮数目。
 4.banner有1张图片的时候不会滚动。
 5.领养宠物会创建一个新的出来。
 6.只有一个领养宠物进兑换页面崩溃。
 ...
 1.内存管理研究。
 2.XX萌星得到XX份口粮，默认0。
 3.开始网络异常，点击重试出现loading。
 4.压缩_1：正常压缩   压缩_2：去掉所有陈旧的代码及类。
 5.摇一摇界面加载完之后再让用户摇一摇。
 
 2015.01.08
 1.所有新浪分享换成可编辑的(求口粮分享弹框，图片详情页，我的萌星，宠物主页，发布照片，摸一摸，遛一遛，送礼物结果弹框)。
 2.清除缓存后边加“打赏提示”开关，勾选“以后不用提醒我了”后，变为off，以后不再提醒。再选择on后，开启提醒。
 3.宠物主页求口粮，宣言。
 
 
 2015.01.09
 1.微信登录用unionid(注册弹框和登录页面)；注册页面用unionid。
 2.根据版本号不同，每次更新都出现引导。
 ...
 2015.01.13
 1.tabBar上显示未读消息数。
 2.通知数量每次有变化时外面的数字也进行相应变化。
 
 2015.01.14
 1.新宠物主页挣口粮历史中正在求口粮的打赏功能。
 2.环信的2个页面的头像，昵称的添加，字段名nickName,other_nickName,tx,other_tx;
 
 2015.01.15
 1.发布照片编辑之后再编辑文字，编辑框报位置不变的bug。
 2.环信2个页面的头像异常bug（待测试）。
 3.照片详情页背面4个按钮，选中状态下再按与相应功能按钮相同。
 
 2015.01.19
 1.瀑布流图片复用。
 2.花金币提示按钮逻辑反了，开关开代表要显示提示，关代表不显示提示。
 3.推送设置点击崩溃，原因是登陆之后更新了个空的optioins。
 4.宠物主页默认信息修改。
 5.充值提示的弹框修改。
 6.新宠物主页捧完之后退出再进还是没捧。
 7.捧最多花费100金币(宠物主页，萌星推荐，选择和搜索萌星)。
 8.启动提示网络异常的“确定”和“取消”按钮去掉“取消”。
 
 
 2015.01.20
 1.B手机登录A账号，A提示已在其他地方登录。
 2.注册或切换账号之后重新登录环信,需要先logoff。
 3.注册无宠物时选择头像之后崩溃。
 4.环信苹果推送。
 mark 发包
 5.捧之后粉丝数+1。
 6.人气榜从宠物主页进定位该宠物。
 7.宠物主页进入赏口粮页面赏的提示逻辑反了。
 8.发照片时如果照片宽高超过2000就处理一下。
 
 
 2015.01.21
 1.摸一摸有时候没有毛玻璃，摸的图片等比裁剪。
 2.赏口粮页面点击照片进入照片详情。
 3.进入（聊天界面、列表页面、发送消息）的时候检测是否登录，未登录提示未登录，是否登录，点击登录之后执行登录并刷新当前页面数据。
 4.微信登录用unionid(注册弹框和登录页面)，3party传unioned和openid（服务器加一个字段wechat_union=unionid供客户端上传），注册页面用unionid。
 5.环信的黑名单。
 6.加圈子如果超过10个开始收费，50起，最多100(选择宠物，搜索宠物，宠物主页，萌星推荐)。
 ...
 4.默认打开消息推送的详情设置。
 5.环信3个人的头像。
 6.求口粮弹框的分享 = 详情页分享  分享的文本如果图片有描述就用描述，没有用默认文本。
 7.萌星卡片宣言和宠物主页宣言旁边加私信按钮。
 8.上传的时候限制图片长宽比和文件大小
 9.挣口粮发布之后打开分享框，正常图片打开图片详情。
 10.点通知栏推送消息进入应用的时候自动跳出消息列表页。
 11.点进聊天界面的时候更新icon上通知数。
 12.FAQ，用户守则。
 13.3个系统人物的头像bug处理。
 14.宠物主页捧之后返回recommend页面捧状态没有刷新。
 
 2015.01.26
 1.友盟事件排查。
 2.环信加自动登录。
 3.图片详情页正面评论之后翻到背面并切换到评论，在背面直接切换。
 
 2015.01.27
 1.图片详情页图片没加载出来的时候点击白图崩溃的问题。
 
 2015.01.28
 1.聊天详情页1，2，3用户的头像用self.headImageView而不是self.imageView。
 2.在会话列表页给详情页传值的时候，判断是不是我发的改成和会话列表相同。
 [conversation.latestMessageFromOthers.ext objectForKey:@"nickname"] == nil
 3.注册弹窗微博微信登陆后刷新center界面。
 4.build号改为和version一致。
 5.注册弹窗微博微信登陆后更新用户code信息及换新登录。
 6.关于我们页面的版本号根据该包的Version号变换。
 7.判断是否是测试的判断改为和当前版本比对。
 8.注册取消和修改资料里的用户名和宠物名的8个字限制。
 9.花费金币提示默认打开。
 ...
 2015.02.02
 1.环信聊天界面打开图片详情之后反面的回复评论无法点击（聊天界面的tap手势覆盖了cell的点击事件）。
 2.照片详情页每次点击增加4M，关闭释放（CoreAnimation,<non-object>会增加0.n M）。
 3.瀑布流在上拉加载更多和的时候和下拉刷新之前，执行图片缓存清理代码。
 4.摸一摸每次+4.6M VM:CG raster data，毛玻璃引起，将_scratchCardView.surfaceImage = nil;
 5.宠物主页释放及图片释放，毛玻璃8M，CoreAnimation 1.4M。
 
 2015.02.03
 1.摸一摸请求图片超时之后无法返回。
 2.聊天界面箭头位置异常问题。
 3.图片详情页评论时输入框位置的问题。
 4.主页4个按钮的赏粮，连点会崩溃，而且按钮变形（动画执行的时候关闭按钮交互，加载数据判断先前的是否加载完，未加载完不再加载，原因是http数据过渡释放）。
 5.摇一摇的viewWillAppear不执行，摇动通知开关问题。
 6.粉丝上拉刷新的问题，改成page控制。
 7.黑名单有2个人的时候，取消第二个就出现小黑屋还没人了。再取消第一个之后有崩溃现象。
 8.没有自己创建的宠物的时候账号里没有收货地址。
 9.注册完之后，第一次进入Login界面时直接返回的情况。
 
 
 2015.02.04
 1.摇一摇送出、次数不足时的分享改成分享网址。
 2.我的萌星页面，每次出现都自动加载下拉刷新。
 3.捧TA素材替换（萌星推荐，选择萌星，搜索萌星），圆形捧TA（宠物主页）。
 4.捧TA动画，及界面释放（在block中添加view或controller要提前声明并加__block，否则会计数+1，造成不释放）。
 5.点赏动画。
 6.给系统发消息会崩溃，other_tx字段为空。
 7.先打开回复系统，再点箭头键盘未收起，执行keyBoardHidden方法。
 8.
 
 2015.02.05
 1.退出圈子按钮颜色左橙右灰，字改成“额，是的”，捧TA弹框“知道啦”--》“没问题”。
 2.捧成功页面的动画在4s上超出屏幕，做下调处理。
 3.萌星周边（我的萌星和宠物主页）。
 4.周边开关逻辑：开关为0时，显示是献爱心；开关为1时，显示买周边，判断url，如果url为空显示弹窗，如果url不为空跳淘宝。
 5.照片发布的分享格式及文字修改。
<<<<<<< HEAD
 
 
 2015.02.06
 1.我的萌星的下拉刷新机制，还是由tabBar控制比较好。
 2.发照片同时分享的机制，分享微博之后回应用会等友盟的代理，这时候关掉发布按钮的交互，避免用户重复提交。
 3.赏口粮的loading效果取消。
 4.邀请按钮 文字“邀请”--》“拉粉丝”。
 5.banner点中可以上下滑，sv.contentSize的高度在获取到图片缩放后的高度之后修改。
 6.买周边和送礼物的图片区分变化。
 7.图片详情页底部4个按钮替换。
 ...
 2015.02.09
 1.没有挣口粮图片时点赏做无反应处理。
 2.发照片的时候取消loading，改为在注册、进入应用、创建或加入新圈子、更改宠物资料、切换用户这5种情况的时候更新本地列表，在切换用户之后要将本地lastPublishAid值设为“0”。
 3.充值界面webView的代理，加载Loading图。
<<<<<<< HEAD
 4.
...
=======
 ...
 会稍有作用。
 5.我的萌星，送礼物和买周边文字复用的问题。
 6.头像注册，修改时自动裁剪中间部分，并且做大于2000判断处理。
 7.探索页面切换到其他tab时自动隐藏搜索结果。
 8.瀑布流加载图片时做大于2000进行缩放的处理。
 
 
 2015.02.10  测服~
 1.探索页面，点开搜索界面之后在背景出一层黑色半透明，点击隐藏搜索。
 2.我的萌星图片加载换成sd，优化界面卡顿现象。
 3.萌星推荐界面图片下载sd，优化卡顿。
 4.带虚线的框，取消复用。
 5.用户卡片释放（求口粮页面，探索页搜索，个人中心，照片详情页反面2个，宠物主页，宠物粉丝，萌星推荐）。
 6.钱包君告急在版本控制之外显示提示充值界面 捧(选择萌星，搜索萌星，萌星推荐，宠物主页，照片详情)  买礼物，送礼物(商城，照片详情，我的萌星，宠物主页)。
 7.照片详情换回白的，首页tab用新的。
 
 
 2015.02.11
 1.瀑布流到照片详情页，先展示照片和描述，之后的该加载加载。
 2.照片详情页释放(瀑布流，我的萌星，照片发布后)。
 3.照片发布时断网之后，打开发布按钮的交互（publishButton.userInter = YES）。
 4.求口粮数据为空时，将是否可刷新数据判断打开（isLoading = NO）。
 5.返回键优化，点击区域增大。
 
 
 2015.02.27
 1.添加2个友盟事件统计，发挣口粮照片和分享挣口粮。
 2.替换我的萌星4个展示图片的默认图和选择萌星的时候萌星4个照片的默认图。
 3.照片详情页评论列表头像不显示bug。
 4.宠物修改资料宠物头像不显示bug。
 5.选择萌星点击推荐闪退bug。
 
 2015.02.28
 1.宠物主页进照片列表，照片用缩略图展示（个别图片重绘失败，图片不显示）。
 2.从照片列表出来后释放掉列表页面。
 3.评论出现错位状况（评论控件高度复用，<15的cell复用>15的label，label文字上下居中，当label过高时导致文字下移）。
 4.照片发布时长款相等而且都>2000时进行缩放处理。
 5.瀑布流图片处理基准提高到2500，考虑到尽量少处理图片，降低cpu负荷。
 
 
 2015.03.02
 1.多个地方分享文字修改，摸一摸和摇一摇分享网址变更。
 2.1403 深情比金坚图片替换。
 3.1402名称 由金饭碗-->萌星合约， 描述修改。
 4.多个地方头像变形情况处理。
 
 2015.03.03
 1.添加友盟事件，register_hint,register_choose,register_suc_pet,register_suc_nopet。
 
 2015.03.04
 1.搜索萌星和人的时候选择完人或宠之后点周围地方收起选项框(搜索控件底层添加透明按钮，点击执行隐藏方法)。
 2.聊天界面点头像打开人的卡片。
 ...
 2015.03.06
 1.照片类型选择页“发布到”一栏显示2个头像，第一个居中对齐界面。
 2.发布照片后返回到我的萌星页面显示照片详情或弹框。
 3.
 
 
 2015.03.09  测试包
 1.收货地址页取本地宠物列表，从中取第一个自己创建的宠物的aid来使用。
 2.发布照片后的展示详情or弹框的判断（我的萌星）。
 3.兑换详情页，商品描述做成动态显示，不要固定4项。
 4.新照片类型判断的添加、赏按钮动态变化成（心，球球，摸）【包括点击底部商量按钮的时候】。
 
 
 2015.03.10
 1.联络官给自己说话加限制，并且把话都归为对方所有，显示跳转箭头。
 2.挣口粮列表图片加载类型变化。
 3.所有Alert_BegFoodViewController弹框创建时传入is_food值，所有发布照片页创建时传入publishType值。
 4.添加4个统计事件topic1_suc, topic1_share_suc, topic2_suc, topic2_share_suc(发布页面也添加上)【包括：发布页面{发布和分享}，发布完的弹框{分享}，再次点击的提示框{分享}，照片详情{分享}】。
 5.强制更新【说明：每天第一次登陆login检查是否需要更新，API外层version值大于当前版本号进行强制更新，API内层version值大于当前版本号进行非强制更新】。
 
 
 2015.03.11
 1.发布照片时选择用户，键盘未收回时返回会和发布页的第三方起冲突导致闪退，在返回前将键盘收起即可。
<<<<<<< HEAD
 
 2015.03.12-13
 1.图片缩略图下载【宠物照片，瀑布流，搜索列表头像，萌宠推荐及人宠头像，赏粮列表及宠物和人的头像，我的萌星头像及图片，人气榜，宠物主页人宠头像，宠物赏口粮列表，粉丝列表头像，动态的照片，个人主页头像，摇一摇头像，摸一摸图片及头像，图片详情图片及头像，挣口粮提示框图片，领养列表及搜索领养头像，聊天列表和聊天界面头像，个人中心头像】。
 2.粉丝列表，领养页面，商城，背包，人气榜，动态页的释放。
 3.发布时勾选新浪之后分享完崩溃。
 4.所有aidigame.com替换为imengstar.com。
 5.图片详情页评论高度范围扩大到1000。
 ...
 *.强制更新闪退（遇到一次），SID没有。
 ...
 6.发照片拍照，内存增长较大，和imageOrientation有部分关系，打开相机和拍照的时候会有警告。
...
=======
>>>>>>> dev-miao
 *.加载图片的时候控制图片占内存大小，试试改变url，直接拿缩略图之类的，缩略图里应该有东西没释放，CG raster data时有时无，瀑布流上拉加载8页左右 imageIO_JPEG_Data就不减了，达到61.16MB，总内存达到86.26MB。
 *.充值页面的释放，玩耍应该也有JS，增长5M，玩耍<non-object>增长20M。
 *.捧成功页面的释放，CoreAnimation 850K。
 
 *.是不是可以添加应用内推送？或者添加上再tabbar界面监听收到消息的代理，不要等用户点击tabBar才检测。
...
=======
 ...
 2015.03.14
 1.选照片类型页面请求menuData漏掉一种情况。
 2.Alert_oneBtnView页面self.type类型改为NSInteger。
 3.修改资料页加入其他种类宠物。
 
 
 2015.03.16
 1.照片类型逻辑改动，本地存储2个列表，包括一个数组[供选择照片页显示顺序使用]和一个可变字典[以label为key，menuModel为值存储]，每次启动时更新数组【默认为@[@"0"]】，遍历返回数组，在字典中添加新label类型，覆盖旧的label类型。
 2.iPad创建宠物第一次点完成不会反回，但已创建成功。
 3.iPad发完挣口粮或话题类型闪退（在removeFromParent之前先retain，之后再release）。
 4.传递imageURL到【照片详情，瀑布流、求口粮列表、宠物照片列表、我的萌星、萌星推荐、宠物动态、宠物历史求口粮】使详情页先显示缩略图。
 5.创建宠物页面人物的头像变形问题。
 6.友盟统计事件，分享的判断，包括 求口粮弹框、照片详情、发布页面，根据发布或分享的是现在话题照片类型的第一个还是第二个来进行统计。
 7.照片详情缩略图到大图，大图的等待加载图用缩略图，避免出现缩略图之后又变白。
 8.发布页面先发布照片，成功后再同步分享。
 9.同步分享到新浪丢失网址链接和小尾巴。
 ...
 2015.03.20
 1.探索页的segment透明度取消，选中和未选中的文字颜色均为白色。
 2.启动图太小了，压失真了2张。
 3.商城购物之后的提示语换成统一调用的方法，不然在提示框未消失前返回会闪退。
 4.照片详情页反面的两个sv上添加的手势，添加代理gestureRecognizer: shouldReceiveTouch:，用NSStringFromClass([touch.view class]判断被点击的是不是UIScrollView，如果是就返回YES，不是就返回NO，返回NO时手势对应方法不执行。
 
 2015.03.23
 1.文章界面的搭建。
 
 2015.03.24
 1.发布照片时图片长，输入文字时拖动图片之后，输入框就弹不出来了【输入框弹出后关闭scrollView的可滑动属性】。
 ...
 2015.03.31 midnoon -- 2015.04.01 pm  一天半  请假
 
 2015.04.02
 1.新图片展示页。添加瀑布流时注意banner对瀑布流起始位置的影响，多个地方会涉及，保存一份全局的瀑布流起始位置。
 2.【渐进式加载】。
 ...
 *.宠物求口粮，摸一摸。
 //*.强制更新闪退（遇到一次），SID没有，login里的blockSelf-->self解决。
 ...
 ***
 2015.2.1
 解决问题：
 1.图片详情页每次点开内存增加，CGImageRef未释放。
 2.first之后将windonw.root修改为tabBar。
 3.增加2个Controller+方法，用于添加和释放背景半透明的controller（照片详情页等用）。
 4.摇一摇界面内存释放（摇出结果界面还未修改完毕）。
 5.httpDownBlock每次用完释放掉其中数据。
 7.赏口粮界面有图片未释放（待解决）。
 9.__block contrller * vc;
   vc->imageView.
 **
 *.我的萌星的4各图片有复用重叠的情况。
 *.聊天界面点箭头跳图片详情页tableView的点击事件失效。
 ==============截止今天================
 *.我的萌星点进去之后不自动下拉刷新而是刷新数据，位置不变。
 *.图片内存管理。
 *.环信搜索框。
 *.环信聊天时只有一条对方消息时，下拉刷新之后时间就不见了。
