...
//    [self addSubview:self.alphaBgView];
    ...
    //600 852
    self.bgImageView = [MyController createImageViewWithFrame:CGRectMake((self.frame.size.width-304)/2.0, (self.frame.size.height-426)/2.0, 304, 426) ImageName:@""];
    [self addSubview:self.bgImageView];
    
    //title
    self.titleLabel = [MyController createLabelWithFrame:CGRectMake(0, 0, 304, 37) Font:17 Text:nil];
    self.titleLabel.textAlignment = NSTextAlignmentCenter;
    [self.bgImageView addSubview:self.titleLabel];
    
    //close
    self.closeBtn = [MyController createButtonWithFrame:CGRectMake(304-20-10, 8.5, 20, 20) ImageName:@"" Target:self Action:@selector(closeClick) Title:nil];
    [self.bgImageView addSubview:self.closeBtn];
    
    //good转动动画
    rollView = [MyController createViewWithFrame:CGRectMake(0, 75/2.0, self.bgImageView.frame.size.width, self.bgImageView.frame.size.height-75/2.0)];
    rollView.layer.cornerRadius = 10;
    rollView.layer.masksToBounds = YES;
    [self.bgImageView addSubview:rollView];
    
    rollImageView = [MyController createImageViewWithFrame:CGRectMake(0, 0, 500*1.2, 940/2*1.2) ImageName:@"send_shine.png"];
    rollImageView.center = CGPointMake(rollView.center.x, rollView.center.y-75/4.0-60);
    [rollView addSubview:rollImageView];
    
    //bad拉伸图片
    badLineImageView = [MyController createImageViewWithFrame:CGRectMake(0, 75/2.0, self.bgImageView.frame.size.width, 157/2.0) ImageName:@"send_notShine.png"];
    [self.bgImageView addSubview:badLineImageView];
    
    //礼物名称背景
    giftNameBg = [MyController createImageViewWithFrame:CGRectMake(-3, 37, 351/2.0, 93/2.0) ImageName:@"send_giftName_bg.png"];
    [self.bgImageView addSubview:giftNameBg];
    
    //
    txBg = [MyController createImageViewWithFrame:CGRectMake((self.bgImageView.frame.size.width-482/2)/2.0, 90-35, 482/2.0, 497/2.0) ImageName:@"send_txBg.png"];
    [self.bgImageView addSubview:txBg];
    
    
    self.giftNameLabel = [MyController createLabelWithFrame:CGRectMake(0, 45, 160, 38) Font:17 Text:nil];
    ...
    self.giftImage = [MyController createImageViewWithFrame:CGRectMake(85+(130-98)/2, 116+15, 98, 83.5) ImageName:@""];
    [self.bgImageView addSubview:self.giftImage];
    
    //rq 165  35
    self.rqLabel = [MyController createLabelWithFrame:CGRectMake(135/2.0, 434/2, 165, 35) Font:17 Text:@""];
    self.rqLabel.textAlignment = NSTextAlignmentCenter;
    [self.bgImageView addSubview:self.rqLabel];
    
    //
    self.pickMore = [MyController createButtonWithFrame:CGRectMake(18, 300, 100, 35) ImageName:@"alert_greenBg.png" Target:self Action:@selector(pickMoreClick) Title:@"再挑一个"];
    self.pickMore.titleLabel.font = [UIFont systemFontOfSize:17];
    [self.bgImageView addSubview:self.pickMore];
    
    self.confirmBtn = [MyController createButtonWithFrame:CGRectMake(304-18-100, 300, 100, 35) ImageName:@"alert_greenBg.png" Target:self Action:@selector(confirmClick) Title:@"确定"];
    ...
    //
    self.actLabel = [MyController createLabelWithFrame:CGRectMake(75, 360, 165, 53-8) Font:13 Text:nil];
