...
@interface ChatViewController ()<UITableViewDataSource, UITableViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, SRRefreshDelegate, IChatManagerDelegate, DXChatBarMoreViewDelegate, DXMessageToolBarDelegate, LocationViewDelegate, IDeviceManagerDelegate,UIAlertViewDelegate>
{
...
-(void)createFakeNavigation
{
    ...
    UILabel * titleLabel = [MyControl createLabelWithFrame:CGRectMake(60, 64-20-15, 200, 20) Font:17 Text:self.other_nickName];
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    [MyControl returnUnreadMessageCount];
}
-(void)examLogin
{
    //判断是否登录了
...
//                [weakSelf hideHud];
                if (!error) {
                    ...
                    if (localPath && localPath.length > 0) {
                        ...
                        weakSelf.messageReadManager.chatVC = self;
                        [weakSelf.messageReadManager showBrowserWithImages:@[url]];
                        return ;
                    }
                }
                [MyControl popAlertWithView:self.view Msg:@"大图获取失败!"];
//                [weakSelf showHint:@"大图获取失败!"];
            ...
            [chatManager asyncFetchMessageThumbnail:model.message progress:nil completion:^(EMMessage *aMessage, EMError *error) {
                if (!error) {
                    ...
                    [MyControl popAlertWithView:self.view Msg:@"缩略图获取失败!"];
...
{
    EMMessage *locationMessage = [ChatSendHelper sendLocationLatitude:latitude longitude:longitude address:address toUsername:_conversation.chatter isChatGroup:_isChatGroup requireEncryption:NO nickName:[USER objectForKey:@"name"] tx:[USER objectForKey:@"tx"] other_nickName:self.other_nickName other_tx:self.other_tx];
...
-(void)sendTextMessage:(NSString *)textMessage
{
    ...
    //    }
//    NSLog(@"%@--%@--%@--%@", self.other_nickName, self.other_tx, self.nickName, self.tx);
    EMMessage *tempMessage = [ChatSendHelper sendTextMessageWithString:textMessage toUsername:_conversation.chatter isChatGroup:_isChatGroup requireEncryption:NO nickName:[USER objectForKey:@"name"] tx:[USER objectForKey:@"tx"] other_nickName:self.other_nickName other_tx:self.other_tx];
    [self addMessage:tempMessage];
}

-(void)sendImageMessage:(UIImage *)imageMessage
{
    EMMessage *tempMessage = [ChatSendHelper sendImageMessageWithImage:imageMessage toUsername:_conversation.chatter isChatGroup:_isChatGroup requireEncryption:NO nickName:[USER objectForKey:@"name"] tx:[USER objectForKey:@"tx"] other_nickName:self.other_nickName other_tx:self.other_tx];
    [self addMessage:tempMessage];
}

-(void)sendAudioMessage:(EMChatVoice *)voice
{
    EMMessage *tempMessage = [ChatSendHelper sendVoice:voice toUsername:_conversation.chatter isChatGroup:_isChatGroup requireEncryption:NO nickName:[USER objectForKey:@"name"] tx:[USER objectForKey:@"tx"] other_nickName:self.other_nickName other_tx:self.other_tx];
    [self addMessage:tempMessage];
}

-(void)sendVideoMessage:(EMChatVideo *)video
{
    EMMessage *tempMessage = [ChatSendHelper sendVideo:video toUsername:_conversation.chatter isChatGroup:_isChatGroup requireEncryption:NO nickName:[USER objectForKey:@"name"] tx:[USER objectForKey:@"tx"] other_nickName:self.other_nickName other_tx:self.other_tx];
    [self addMessage:tempMessage];
}
